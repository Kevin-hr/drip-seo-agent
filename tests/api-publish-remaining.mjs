...
const TARGETS = [
  '536027544723736', // Polo blocklist
  '536027544674326', // Polo blocklist
  '536027517605911', // Sp5der blocklist
  '536027239001368', // Balenciaga
  '536027070065427', // Nike SB Blazer
];
...
for (const id of TARGETS) {
  try {
    ...
    if (row.IsShow === true) { console.log(`${id}: already published`); continue; }
    ...
    if (resp.success === true) {
      console.log(`${id}: SUCCESS ${row.Name?.slice(0,40)} | Content: ${(row.Content||'').length} chars`);
    } else {
      console.log(`${id}: FAIL ${JSON.stringify(resp).slice(0,200)}`);
    }
    await page.waitForTimeout(1200);
  } catch (e) {
    console.log(`${id}: ERROR ${String(e).slice(0,200)}`);
  }
}
await browser.close();
