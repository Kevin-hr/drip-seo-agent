  ...
  const results = await page.evaluate(async (batch) => {
    ...
    for (const p of batch) {
      try {
        ...
        out.push({ id: String(p.productId), name: p.name, isShow: row?.IsShow });
      } catch(e) { out.push({ id: String(p.productId), name: p.name, error: String(e).slice(0,50) }); }
    }
    return out;
  }, batch);
  for (const r of results) if (r.isShow !== true) unpublished.push(r);
  process.stdout.write(`\rChecked ${Math.min(i+30, products.length)}/${products.length}`);
}
console.log('\n\nUNPUBLISHED:');
for (const u of unpublished) console.log(JSON.stringify(u));
await fs.writeFile('data/runs/v45-batch-2026-09-23/minimal-run/unpublished-final.json', JSON.stringify(unpublished, null, 2));
await browser.close();
