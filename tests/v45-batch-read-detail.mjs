...
// Target: VERIFY products with a SKU, Nike first (cleanest evidence), then others
const targets = queue.products
  .filter((p) => p.disposition === 'VERIFY' && p.sku)
  .sort((a, b) => {
    const brandRank = { nike: 0, 'off-white': 1, balenciaga: 2, gucci: 3, bape: 4, 'louis-vuitton': 5, other: 9 };
    return (brandRank[a.brandBucket] ?? 9) - (brandRank[b.brandBucket] ?? 9);
  });
...
console.log(`Selected ${batch.length} of ${targets.length} SKU-bearing VERIFY products (offset ${offset})`);
    ...
    try {
      const detail = await page.evaluate(async (pid) => {
        ...
        return {
          ...
          marketPrice: row.MarketPrice || row.SalePrice || '',
      ...
      details.push({ ...detail, brandBucket: p.brandBucket, sku: p.sku, queueReason: p.reason });
      console.log(`  read ${id} | imgs=${detail.imageCount} | ${detail.name.slice(0, 60)}`);
      break;
    } catch (e) {
      if (attempt === 3) { details.push({ id, error: String(e), brandBucket: p.brandBucket, sku: p.sku }); console.log(`  FAIL ${id}: ${e}`); }
...
await fs.writeFile(path.join(runDir, 'batch-detail-read.json'), `${JSON.stringify(details, null, 2)}\n`);
console.log(`Wrote ${details.length} details to batch-detail-read.json`);
